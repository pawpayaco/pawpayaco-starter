...
export default async function handler(req) {
  ...
  // -------- GET: status probe (/api/claim?u=UID)
  if (req.method === 'GET') {
  ...
  // -------- POST: claim a business for this UID
  if (req.method === 'POST') {
    let body = {};
    ...
    // create binding (store affiliate_url so GET can redirect next time)
    ...
    if (!bindRes.ok) {
      ...
      return json({ error:'bind_fail', detail }, 500);
    }

    // mark business as claimed (optional)
